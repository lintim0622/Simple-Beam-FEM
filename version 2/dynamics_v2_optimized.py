...
class DynamicsSolver:
    def __init__(self, msh, material, t_end, dt, lumped_mass=False, alpha=0.0, beta=0.0):
        ...
        self.C_ff = alpha * self.M_ff + beta * self.K_ff

        # 建立 DoF 對應查表
        self.gid_to_idx = {gid: i for i, gid in enumerate(self.free_dofs)}

        # 預組裝 A_eff（Newmark）
        a1 = 1 / (0.25 * self.dt**2)
        a2 = 0.5 / (0.25 * self.dt)
        self.A_eff = csr_matrix(self.M_ff * a1 + self.C_ff * a2 + self.K_ff)
    ...
    def step_newmark(self, i, F_f, beta=1/4, gamma=1/2):
        ...
        self.U[i + 1] = spsolve(self.A_eff, rhs)
